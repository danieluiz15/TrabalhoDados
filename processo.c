...
int calcularDiasEntreDatas(const char *filename, const char *id_processo)
{
    ...
    while (fgets(linha, sizeof(linha), fp))
    {
        ...
        if (token && strcmp(token, id_processo) == 0)
            achar = 1;
...
// Função auxiliar 
int stringParaInt(const char *texto)
{ 
...
// Item 11 para calcular a meta 1
...
// Item 12 - Gerar CSV com processos julgados
